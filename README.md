...
Its goal is to receive an `url` as a parameter and return a 8 digit code, which can be user later on as a short URL.

To accomplish this goal, the application was developed in such a way that it allows the use multiple repositories and serializers, via separation of logic and specific implementations, such as:
...
The applications expects some environment variables, which can be found in [docker-compose.yaml](docker-compose.yaml). One of great importance is `DATABASE_ENGINE` and it can assume the values `redis` and `mongo`.
